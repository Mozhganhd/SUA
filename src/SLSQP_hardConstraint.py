...
def run_optimization_with_group_constraints(product_df: pd.DataFrame, simulated_demand: np.ndarray, macro_cap: float):
    ...
    # Macro-level constraint
    constraints.append({'type': 'ineq', 'fun': lambda x: macro_cap * total_demand - np.sum(x)})

    # Group-level minimum surplus constraints
    ...
    for group_id, indices in group_map.items():
        ...
        def make_group_constraint(idx, group_need):
            return lambda x, idx=idx, group_need=group_need: np.sum(x[idx]) - group_need

        constraints.append({
            'type': 'ineq',
            'fun': make_group_constraint(idx, group_need)
        })
    ...
    for group_id, indices in product_df.groupby('Substitutability group').groups.items():
        ...
        group_need = overflow[idx].sum()
        print(f"Group {group_id}: Surplus = {group_surplus:.2f}, Needed(overflow) = {group_need:.2f}, Gap = {group_need - group_surplus:.2f}")
