...
def load_and_clean_data(filepath: str):
    ...
    product_df.columns = ['Demand', 'Variancegroup', 'Margin', 'COGS', 'Capacity', 'Substitutability group']
    product_df['ProductID'] = [f'P{i}' for i in range(len(product_df))]

    product_df['Demand'] = pd.to_numeric(product_df['Demand'], errors='coerce')
    product_df['Variancegroup'] = pd.to_numeric(product_df['Variancegroup'], downcast='integer', errors='coerce')
    ...
    product_df = product_df.rename(columns={'Variancegroup': 'DemandVarGroup'})
...
def define_objective(product_df, demand, margin, cogs, simulated_demand, max_surplus):
    def objective(surplus):
        ...
        for group_id, indices in sub_groups.items():
            ...
            if group_surplus < group_need:
                penalty += (group_need - group_surplus)**2  # quadratic penalty
        return -np.mean(total_profit) + 1e-1 * penalty
...
def run_optimization_with_group_constraints(product_df: pd.DataFrame, simulated_demand: np.ndarray, macro_cap: float):
    ...
    objective_fn = define_objective(product_df, demand, margin, cogs, simulated_demand, max_surplus)
...
def save_results_and_visualize(product_df, result, max_surplus, simulated_demand, output_dir="final_with_constraints5"):
